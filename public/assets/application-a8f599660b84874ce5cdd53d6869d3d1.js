...
(function() {
  $(function() {
    $('header i:first').tipsy({
      fade: true,
      gravity: 'e'
    });
    return $('header i:last').tipsy({
      fade: true,
      gravity: 'w'
    });
  });
...
(function() {
  ...
  $(function() {
    var fetchPost, fetchPosts, initialURL, popped, prependDimmer;
    window.changeView = function(html) {
      var callback;
      $('#spinner').fadeIn('fast');
      $('.arrow').fadeOut('fast');
      callback = function() {
        $('#main').html(html).removeClass('transition');
        return $('#spinner').fadeOut('fast');
      };
      return setTimeout(callback, 1250);
    };
    window.dim = function(lightSwitch) {
      if (!($('.dim').length > 0)) {
        prependDimmer();
      }
      if (!lightSwitch) {
      ...
      if (lightSwitch) {
        return $('.dim').fadeIn('fast');
      } else {
        return $('.dim').fadeOut('fast');
      }
    };
    ...
    prependDimmer = function() {
      return $('#main').prepend('<div class="dim"></div>');
    };
    fetchPosts = function() {
      return $.ajax('/').done(function(html) {
        changeView(html);
        return $('#posts').packery();
      });
    };
    fetchPost = function(url) {
      return $.ajax(url).done(function(html) {
        return changeView(html);
      }).fail(function() {
        return console.log('failed to load post.');
      });
    };
    popped = (__indexOf.call(window.history, 'state') >= 0);
