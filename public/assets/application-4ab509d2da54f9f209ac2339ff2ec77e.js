...
(function() {
  $(function() {
    var getListingLayout, prependDimmer;
    window.updateMainCanvas = function(html) {
    ...
    $(window).on('popstate', function(event) {
      if (event.originalEvent.state !== null) {
        return getListingLayout();
      }
    });
    return getListingLayout = function() {
      if ($('.posts').length > 0) {
        return false;
      }
      return $.ajax('').done(function(html) {
        return updateMainCanvas(html);
      }).fail(function() {
        return console.log('Failed to load main layout.');
      });
    };
  });
